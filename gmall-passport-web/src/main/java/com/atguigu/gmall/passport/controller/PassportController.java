...
public class PassportController {
    ...
    public String login(UmsMember umsMember, HttpServletRequest request){
        String token = "";
        // 调用用户服务验证用户名和密码
        UmsMember userMember = userService.login(umsMember);
        if(userMember!=null){
            // 登录成功
            // 用jwt制作token
            String memberId = userMember.getId();
            String nickname = userMember.getNickname();
            ...
            token = JwtUtil.encode("2020gmall110105", userMap, ip);
            // 将token存入redis一份
    ...
    public String vlogin(String code,HttpServletRequest request){
        ...
        // 生成jwt的token并且重定向到首页，携带token
        String token = null;
        String memberId = umsMember.getId(); // rpc的主键返回策略会导致里面是空值
        ...
        // 将token存入redis一份
        userService.addUserToken(token,memberId);
        return "redirect:http://127.0.01:9401/index?token="+token;
    }
    ...
    public String verify(String token, String currentIp, HttpServletRequest request){
        ...
        Map<String, Object> decode = JwtUtil.decode(token, "2020gmall110105", currentIp);
        if(decode != null){
